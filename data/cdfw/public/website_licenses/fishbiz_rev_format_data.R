...
# Read data
data_orig <- read.csv(file.path(datadir, "tabula-2000s Revenue Fish Business.csv"), na.strings=c("N/A", ""))
...
data <- mutate(data, filename="tabula-2000s Revenue Fish Business.csv")
data <- data_orig %>% 
  mutate(decade="2000s") %>% 
  mutate(filename="tabula-2000s Revenue Fish Business.csv")
...
ggplot(data=data1, mapping=aes(x=year, y=revenues_usd/1e6, fill=license)) +
  ...
  # Labels
  labs(x="Year", y="Revenues (USD millions)", title="Revenues from fish business licenses, 2000-2009") +
  # Axes
