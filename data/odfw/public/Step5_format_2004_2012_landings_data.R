...
# Read data
data_orig <- readxl::read_excel(file.path(workdir, "2004_2012_landings_data_messy.xlsx"))
...
# Format data
data_full <- data_orig %>% 
  select(-c("Total 2", "Check")) %>% 
  gather(key="month", value="value", 7:ncol(.)) %>% 
  mutate(value=value %>% gsub("NA", "", .) %>% as.numeric())
...
tots_check <- tots_rep %>% 
  ...
  filter(!is.na(value_diff) & value_diff!=0) # check for NAs off temporarily
...
g <- ggplot(tots_check1, aes(x=year, y=month, fill=value_diff)) +
  ...
  labs(x="", y="") +
  # Legend
  scale_fill_gradient2(name="Difference in apparent\nand repoted values (lbs)",
                       low="darkred", high="navy", mid="grey80", midpoint=0, na.value="grey30") +
...
# Export data
saveRDS(data, file=file.path(workdir, "2004_2012_landings_data_clean.Rds"))
